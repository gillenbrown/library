...
One final note here is that ADS can take a bit of time to get the daily arXiv papers into its system. Since my code relies on ADS, the most recent arXiv papers may fail when you try to add them. You may need to try to add them later that day or tomorrow. 

Importing papers from a BibTeX file is straightforward. You'll be prompted to select a BibTeX file, then the code will parse the file. Any entries that were not successfully imported are written to a file with the reason that the import failed for each paper. During the import, a new tag will be created and all papers from this BibTeX file will be added to the new tag. This tag can be renamed like any other.
...
On startup, the code will detect whether your OS is using dark or light mode, and match the interface theme accordingly. You can switch the theme by clicking the Library logo in the top left. Currently the code cannot detect any OS theme changes after startup.
